	...
	<view>
		<view class="top_cont">
			<view class="search_cont">
				...
				<view class="chat_type" v-if="active_top == 0">
					...
					<view @click="version(1000)" :class="[active == 1000 ? 'active_tabs' : 'tabs']">火山云雀</view>
					<view @click="version(1001)" :class="[active == 1001 ? 'active_tabs' : 'tabs']">腾讯混元</view>
		...
		<view class="chat_cont">
			<scroll-view class="list_ul" scroll-y="true" v-if="chatList.length > 0"
				:style="[{marginTop:(active_top == 1? '268rpx':'340rpx')}]">
				...
				<view class="ul_style" v-for="(item, index) in chatList" :key="index"
					@click="checkDetail((item.chat_type < 13||item.chat_type==1000||item.chat_type==1001) && item.chat_type != 3 ? item.session_code : item.image_code)">
					<view class="li_style">
						<view class="li_left" v-if="active_top == 1">
							<image class="li_img"
								:src="getImgUrl((item.chat_type < 13||item.chat_type==1000||item.chat_type==1001) && item.chat_type != 3 ? item.content : item.result_image)">
							</image>
						</view>
						<view class="li_right">
							<view class="li_top">
								...
								<text class="top_r"
									v-if="item.chat_type == 1000 && item.model == 'skylark-chat' ">火山云雀-chat</text>
								<text class="top_r"
									v-if="item.chat_type == 1000 && item.model == 'skylark2-pro-4k' ">火山云雀-4k</text>
								<text class="top_r"
									v-if="item.chat_type == 1000 && item.model == 'skylark-lite' ">火山云雀-lite</text>
								<text class="top_r"
									v-if="item.chat_type == 1000 && item.model == 'skylark-plus' ">火山云雀-plus</text>
								<text class="top_r"
									v-if="item.chat_type == 1000 && item.model == 'skylark-pro' ">火山云雀-pro</text>
								<text class="top_r"
									v-if="item.chat_type == 1001 && item.model == '标准版' ">腾讯混元-标准版</text>
								<text class="top_r"
									v-if="item.chat_type == 1001 && item.model == '高级版' ">腾讯混元-高级版</text>
		...
		methods: {
			...
			getChatList() {
				...
				if (this.active_top == 0) {
					...
					if ((this.active < 13 || this.active == 1000 || this.active == 1001) && this.active != 3) {
					...
					success: function(res) {
						if (res.confirm) {
							...
							if (that.active == 4 || that.active == 5 || that.active == 7 || that.active == 8 ||
								that.active == 10 || that.active == 12|| that.active == 1000|| that.active == 1001) {
								uni.navigateTo({
