<template>
	<view>
          ...
          <!--  <view class="product_rule" v-if="active == 0 && !isOne">
			...
			<view class="bottom">
				...
				<view class="bottom_cont">
					<view class="bottom_buy" @click="pay(1)" v-if="!isIos">立即购买</view>
					<view class="bottom_buy" @click="pay(1)"  v-if="isIos && isOpen">立即购买</view>
					<view class="slove_t" v-if="isIos && !isOpen">由于相关规范，iOS功能暂不可用</view>
			...
			<view class="container_cont">
				<view class="container_item" style="height: 250upx;padding-top: 40rpx;" v-for="(item, index) in currentList" :key="index"
					...
					<view  v-if="item.prod_id == '18'" style="height:9%;"></view>
					<view v-else style="height: 38%;"></view>
					<view class="cont_cs" :style="[{background:(active_all === index ? '#1F64FF':'#E8EAEC')}]">
						{{ item.prod_desc.valid }}</view>
		...
		<view class="bottom">
			...
			<view class="bottom_cont">
				<view class="bottom_buy" @click="pay(2)" v-if="!isIos">立即购买</view>
				<view class="bottom_buy" @click="pay(2)"  v-if="isIos && isOpen">立即购买</view>
				<view class="slove_t" v-if="isIos && !isOpen">由于相关规范，iOS功能暂不可用</view>
				<view class="bottom_desc">
					<text>购买即视为同意</text>
					<text style="color: #1F64FF;" @click="checkAgree">《商品服务协议》</text>
		...
		onShow() {
			...
			uni.getSystemInfo({
			  success: function (res) {
			    console.log(res,5589);
				let platform = res.osName;
				if(platform == 'ios') {
					that.isIos = true
				} else {
					that.isIos = false
