<template>
	<view class="home">
		...
		<view class="apply_cont">
			<view class="cont_text" v-if="current > 0" style="padding-top: 288rpx;">
					...
					<scroll-view scroll-y="true" @scrolltolower="scrolltolower" :style="{height: listHeight +'px'}">
						<view class="hotWrods">
							<view v-if="promptsList.length > 0" class="hot_cont">
								<view @click="toCreate"
									style="width: 42%;display: flex;flex-direction: column;align-items: center;justify-content: center;
background: #FFFFFF;box-shadow: 0px 2.23993px 7.83975px rgba(51, 51, 51, 0.1);border-radius: 8px;margin-bottom: 10px;padding: 46rpx 26rpx;">
									<u-icon name="plus" color="#2979ff" size="20"></u-icon>
									<view style="color: #2979ff;font-size: 36rpx;line-height: 200%;">创作</view>
								</view>
			...
			<!-- 私享定制 -->
			<view v-if="current == 0" style="margin-top: 214rpx;">
				...
				<view class="manage_info" v-if="isLogin">
					<view class="notice" v-if="messageList.length > 0">
						<u-notice-bar :text="messageList" color="#1E211F" bgColor="#EBF1FF" mode="closable"
							direction="column" @click="toNotice" duration='3000'></u-notice-bar>
					</view>
					<!-- 我的信息 -->
					<view class="info_me"
					...
					<view class="info_me"
						...
						<view class="me_title" style="margin-bottom: 20upx;">
							<view class="title_left">私享AI导师</view>
							<view class="title_right" @click="toManage">
								<text class="right_texts">管理</text>
						...
						<view class="other_cont">
							<view class="cont_left" v-for="(item, index) in otherList" :key="index"
								...
								<image :src="getImgUrl(item.avatar)" mode="" class="left_img"
									...
									<image :src="qhImg" mode="" class="qh_img"
										@click="addMentor(meList.me_id,index,item.tutor_id)">
							</view>
						</view>
					</view>
				...
				<view style="padding: 20upx;">
					<view class="mentor">
						<view class="mentor_title">热门推荐</view>
						...
						<view style="color: #1F64FF;font-size: 28rpx;text-align: center;" v-if="mentorList.length<6"
							@click="getTutor(1)">展开全部</view>
						<view style="color: #1F64FF;font-size: 28rpx;text-align: center;" v-else @click="getTutor()">收起
						</view>
					</view>
				</view>
	...
	export default {
		...
		onShow() {
			...
			if (this.current == 0) {
				uni.createSelectorQuery().select(".head_index").boundingClientRect(function(
					...
					if (that.current == 3) {
						that.keyHeight = parseInt(that.topHeight) + 40;
			...
			let name = uni.getStorageSync('applyName')
			this.tip = 0; //每次切换选中第一个
			let subscript = this.tabList.findIndex(item => item.name === name);
			if (name == '热门场景') {
				this.current = subscript;
				this.keyHeight = parseInt(this.topHeight) + 80
				this.listHeight = parseInt(this.msgListHeight) - parseInt(this.keyHeight)
				this.select_text = index;
				this.select_text2 = 0;
				this.select_text3 = 0;
				this.select_text4 = 0;
				this.select_text5 = 0;
				...
				this.dwList = []
				// this.clear()
			} else if (index == 'AI数字孪生') {
				// uni.setStorageSync('applyCurrent', this.current)
				this.current = 0
		...
		methods: {
			...
			getTutor(num) {
				...
				getTutor(val).then(res => {
					if (res.code == 20000) {
						this.exArr1 = [];
						this.exArr2 = [];
						this.mentorList = res.data.slice(0, 5);
						if (num == 1) {
			...
			tabsChange(e) {
				let index = e.tab_id;
				this.tip = 0; //每次切换选中第一个
				let subscript = this.tabList.findIndex(item => item.tab_id === index);

				// uni.setStorageSync('applyCurrent', this.current)
				if (index == '489956304003654') {
					// uni.setStorageSync('applyCurrent', this.current)
					this.current = 0
					// this.keyHeight = parseInt(this.topHeight)
					// this.listHeight = parseInt(this.msgListHeight) - parseInt(this.keyHeight)
					...
					// uni.setStorageSync('applyCurrent',1)
					this.current = subscript;
					// this.keyHeight = parseInt(this.topHeight) + 40
					// this.listHeight = parseInt(this.msgListHeight) - parseInt(this.keyHeight)
					this.select_text = index;
