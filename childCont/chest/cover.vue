<template>
	<view style="background-color: #FEFEFE;height: 100vh;">
		...
		<view class="bigBox" v-if="picSrc!=''">
			<image class="imgBigImg" mode="aspectFit" :src="picSrc"></image>
			<image class="imgUpImg" @click="previewImage" :src="icon_img" mode="aspectFit"></image>
			<image class="imgDownImg" @click="savePhoto(src)" :src="icon_img1" mode=""></image>
		</view>
		<view class="tips">
			<text class="tip_m">Tips:自动检测与擦除视频中的常见标志，可用于视频素材的再创作
</text>
		</view>
		<view class="bottom">
								...
								if (res.code === 20000) {
									...
									if (res.data != null) {
										if (res.data[0].status != 1) {
											...
											this.picSrc=global.baseImg+'/'+res.data[0].result_list[1]
			...
			toSlove() {
				...
				uni.chooseMedia({
					...
					success(res) {
						...
						if (fileExtension === 'mp4' || fileExtension === 'flv' ||
							fileExtension === 'ts' || fileExtension === 'rmvb') {
							...
							that.$api.msg('只允许上传 .mp4、flv、.ts、.rmvb 格式的视频')
							return
						}
					}
				})
