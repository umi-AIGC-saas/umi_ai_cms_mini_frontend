<template>
	<view>
		...
		<view class="chat_cont">
			...
			<!-- 分组选择 -->
			<u-picker :show="roleShow" :columns="roleList" v-model="roleIndex" keyName="label"
				@cancel="roleShow = false" @confirm="roleSubmit"></u-picker>
			<!-- 分类选择 -->
			<u-picker :show="sortShow" :columns="sortList" v-model="sortIndex" keyName="label" 
				@cancel="sortShow = false" @confirm="sortSubmit"></u-picker>

			<!-- form弹窗 -->
			<u-popup :show="formShow" mode="bottom" :round="10" closeable="true" zIndex="10000"
				:overlayStyle="{'z-index': '9999'}" @close="closePopup" @open="formShow = true">
				<view class="dia_cont">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			if (option.val) {
				let agent = JSON.parse(option.val)
				console.log(agent, 774);
				...
				this.setValue = agent.agent_prompt
				...
				groups_model(val).then(res => {
					...
					if (res.code == 20000) {
						if (uni.getStorageSync('gro')) {
							...
							const isAllMatched = Object.values(group_tree).every(item => item.id ===
								groArr.id);
							if (!isAllMatched) {
								let data = [{
