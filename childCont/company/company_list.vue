<template>
	<view>
		<view class="cont_title">点击切换个人或/团队/企业</view>
		<!-- 内容 -->
		<view class="chat_cont">
			<scroll-view class="list_ul" scroll-y="true" v-if="companyList.length > 0">
				<view class="ul_style" v-for="(item, index) in companyList" :key="index" @click="checkDetail(item, index)">
					<view class="li_style">
						<view class="ul_img" v-if="item.icon_url != ''"><image class="avatar_img" :src="getImgUrl(item.icon_url)"></image></view>
						<view class="ul_img" v-else><view class="text_img">{{ item.company_name.trim().slice(0,1) }}</view></view>
						<view class="ul_right">
							<view class="li_top">
