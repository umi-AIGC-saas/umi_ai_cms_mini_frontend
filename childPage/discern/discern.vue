<template>
	<view id="chat">
			...
			<u-list :height="scrollHeight" :scrollTop="scrollTop" style="padding: 0 18upx;">
				<view class="qwert" :style="{'padding-bottom':Height+'px'}">
					<u-list-item v-for="item,index in messageList[listName]" :key="index" class="listItem"
										...
										<veiw v-else>
											...
											<view :style="[{marginBottom:(imgList.length > 0 ? '100rpx':'0')}]">
												<u-row gutter="20">
													...
													<u-col span="4" v-if="messageList[listName].length > 2 && messageList[listName][messageList[listName].length - 1].loading">
														<u-button type='primary'
						...
						<view class="link_l" style="margin-left: 100rpx;" v-if="imgList.length==''&&messageList[listName].length <=2">
		...
		<view class="bottomInput" ref="textInput" :style="{bottom:Height+'px'}" style="padding-bottom: 30rpx;">
			<view class="btn_link">
				<view class="link_l">
				...
				<view class="link_l" v-if="imgList.length==''&&messageList[listName].length <=2">
					<view class="l_chat" @click="takePhoto">
						<u-icon name="camera" color="#1F64FF" size="20"></u-icon>
						<text class="l_text">AI识图</text>
					</view>
	...
	export default {
		data() {
			return {
				...
				messageList: {
					'nine': [{
						talk: `<span style="font-size: 17px">我可以识别图片中的各种物体、场景或人物方便用户从图片中提取需要信息</span><br /><span style="font-size:14px;color:#9B9B9F;">9w算力/1k token</span>`,
						type: 'assistant'
					}]
				},
		...
		methods: {
					...
					this.requestTask_nine.onChunkReceived(response => {
						...
						arr.map((item => {
							...
							if(JSON.parse(item).code) { 
								console.log(JSON.parse(item).code,'JSON.parse(item).code');
			...
			toHistory(){
				uni.navigateTo({
					url:'/childPage/discern/discern_his'
				})
			},
