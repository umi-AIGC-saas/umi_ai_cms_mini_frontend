<template>
	<view>
		...
		<view class="chat_cont">
			<scroll-view class="list_ul" scroll-y="true" v-if="chatList.length > 0" :style="[{marginTop:('110rpx')}]">

				<view class="ul_style" v-for="(item, index) in chatList" :key="index"
					@click="checkDetail(item.session_code)">
					<view class="li_style">
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							uni.setStorageSync('tabStatus', 1002)
