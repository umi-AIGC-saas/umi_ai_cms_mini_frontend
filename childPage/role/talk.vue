<template>
	<view>
		...
		<view class="chat_cont">
			...
			<scroll-view class="list_ul" scroll-y="true" v-if="chatList.length > 0" style="margin-top: 288rpx;">
					...
					<view class="li_style">
						...
						<view class="li_right">
							<view class="li_top">
								...
								<!-- <text class="top_r">{{ item.chat_type == 0 ? '无忧秘书智脑35' : '无忧秘书智脑40' }}</text> -->
								...
								<text class="top_r" v-if="item.chat_type == 10 ">通义千问</text>
								<text class="top_r" v-if="item.chat_type == 11">商汤日日新</text> -->
							</view>
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							if(that.active == 0 || that.active == 1 || that.active == 4 || that.active == 1000|| that.active == 1001|| that.active == 5 || that.active == 7 || that.active == 8|| that.active == 10 || that.active == 11) {
								uni.navigateTo({
