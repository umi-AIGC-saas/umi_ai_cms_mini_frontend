<template>
	<view>
		<view class="top_cont">
			<view class="search_cont">
				<u-tabs :list="list" lineWidth='40' lineHeight='2' lineColor='#1f64ff'
					...
					:inactiveStyle='{"font-size":"26rpx"}' :current='current'></u-tabs>
			...
			<scroll-view class="list_ul" scroll-y="true" v-if="infoList.length > 0">
				<view class="ul_style" v-for="(item, index) in infoList" :key="index">
					<view class="li_style" @click="toDetail(item)">
						...
						<view class="li_cont">
							<view class="li_left">
								<image class="li_img" :src="item.character_avatar" v-if="item.character_avatar != ''">
								</image>
							...
							<view class="cont_text">
								<view class="text_t">
									<view class="t_left" v-if="item.character_avatar != ''">{{ item.character_name }}
									</view>
									<view class="t_left" v-else>{{ item.assistant_title }}</view>
									<view class="li_top">
										<!-- <text class="top_text">正在审核中，审核后将在平台展示</text> -->
										<view class="icon_text" @click.stop="checkTip(item.refuse_reason)"
											v-if="item.status == 2"><u-icon name="question-circle"
												font-size="30"></u-icon></view>
										<view class="icon_text" @click.stop="editChat(item.question_id)"
											v-if="item.status == 2"><u-icon name="edit-pen-fill"
												font-size="30"></u-icon></view>
										<view class="icon_text" @click.stop="delChat(item.question_id)"><u-icon
												name="trash" font-size="30"></u-icon></view>
									</view>
								</view>
								<view class="text_c">
									<text class="text_b"
										v-if="item.character_avatar != ''">{{ item.character_greetings }}</text>
		...
		<!-- 模块弹窗 -->
		<u-popup :show="selectDialog" mode="bottom" :round="10" closeable="true" @close="selectDialog = false"
			@open="selectDialog = true">
			<view class="dia_cont">
		...
		<!-- 模块选择 -->
		<u-picker :show="industryShow" :columns="industryList" v-model="industryIndex" keyName="label"
			@cancel="industryShow = false" @confirm="industrySubmit"></u-picker>

		<!-- 职业选择 -->
		<u-picker :show="occupationShow" :columns="occupationList" v-model="occupationIndex" keyName="name"
			@cancel="occupationShow = false" @confirm="occupationSubmit"></u-picker>

		<!-- 子职业选择 -->
		<u-picker :show="secShow" :columns="secList" v-model="secIndex" keyName="name" @cancel="secShow = false"
			@confirm="secSubmit"></u-picker>

		<!-- 从业时间选择 -->
		<u-picker :show="durationShow" :columns="durationList" v-model="durationIndex" keyName="emp_duration_name"
			@cancel="durationShow = false" @confirm="durationSubmit"></u-picker>

		<!-- 地位选择 -->
		<u-picker :show="expertiseShow" :columns="expertiseList" v-model="expertiseIndex" keyName="name"
			@cancel="expertiseShow = false" @confirm="expertiseSubmit"></u-picker>

		<!-- 分类选择 -->
		<u-picker :show="moudelShow" :columns="moudelList" v-model="moudelIndex" keyName="name"
			@cancel="moudelShow = false" @confirm="moudelSubmit"></u-picker>

		<!-- 搜索选择 -->
		<u-picker :show="infoShow" v-model="searchValue" :columns="searchList" keyName="label"
			@cancel="infoShow = false" @confirm="pickSelect"></u-picker>
	</view>
</template>
		...
		methods: {
			...
			getInfoList() {
				...
				getUserModel(val).then(res => {
					if (res.code == 20000) {
						...
						const data = res.data
						if (this.is_status == 0) {
							this.infoList = data.filter(item => item.status == "0")
						} else if (this.is_status == 2) {
							this.infoList = data.filter(item => item.status == "2")
						} else if (this.is_status == 3) {
							this.infoList = data.filter(item => item.status == "3" || item.status == "1")
						} else {
							this.infoList = data
						}
			...
			industrySubmit(e) {
				...
				if (this.industryIndex == 4) {
					...
					let data = {
						industry_id: this.industryIndex
					}
					this.getModules(data)
				}
			},
			...
			durationSubmit(e) {
				...
				let data = {
					...
					occu_id: this.occupationIndex,
					sub_occu_id: this.secIndex,
					emp_duration_id: this.durationIndex
				}
			...
			expertiseSubmit(e) {
				...
				let data = {
					...
					sub_occu_id: this.secIndex,
					emp_duration_id: this.durationIndex,
					expertise_level_id: this.expertiseIndex
				}
				this.getModules(data)
			},
