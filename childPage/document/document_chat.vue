<template>
	<view id="chat">
		<scroll-view ref="box" class="u-page" scroll-with-animation scroll-y="true" id="textadd"
			...
			<u-row align="top" gutter="5">
				<u-col span="1.5" textAlign="center" align="top" justify="top">
					<image style="height: 38px;width: 38px;  border-radius: 50%;" :src="glmImg">
					</image>
				</u-col>
				<u-col soan='9'>
					<view class="tips">
						...
						<view v-if="isDoc" style="display: flex;justify-content: space-between;">
							<view class="document_type">支持文件类型:pdf</view>
							<view class="document_type">6w算力/页</view>
						</view>
		...
		<view class="bottomInput bottom_p" :style="{bottom:bottomTop+'px'}">
			...
			<view class="btn_link" style="display: flex;justify-content: space-between;">
				<view class="link_l">
					<view class="l_chat" style="margin-right: 15rpx;" @click="newChatSpeak"
						v-if="messageList[listName].length > 1 && messageList[listName][messageList[listName].length - 1].loading">
	...
	export default {
		...
		methods: {
			...
			showFun() {
				...
				if (this.listName == 'three' || this.listName == 'five' || this.listName == 'three' || this.listName ==
					'seven') {
