	...
	<view>
		...
		<view class="top_cont">
			...
			<!-- 	<view class="chat_type">
					<view @click="version(4)" :class="[active == 4 ? 'active_tabs' : 'tabs']">普通版</view>
					<view @click="version(1)" :class="[active == 1 ? 'active_tabs' : 'tabs']">高级版</view>
				</view> -->

			</view>
		</view>
		<view class="chat_cont" style="margin-top: 120upx;">

			<scroll-view class="list_ul" scroll-y="true" v-if="chatList.length > 0">
				<view class="ul_style" v-for="(item, index) in chatList" :key="index">
					<view class="li_style" @click="checkDetail(item.h_code)">
						<view class="ul_right">
							<view class="li_top">
								<text class="top_l">{{ item.title }}</text>
								<image style="width: 48upx;height: 48upx;" :src="delImg"
									@click.stop="delPhoto(item.h_code)"></image>
							</view>
							<!-- <view class="li_btn">
								<view class="btn_l">话题</view>
								<view class="btn_r" style="display: flex;align-items: center;">
									<view >{{  }}</view>
									<image :src="right_img" style="width: 48rpx;height: 48rpx;"></image>
								</view>
							</view>
							<view class="li_btn">
								<view class="btn_l">时长</view>
								<view class="btn_r">1分11秒</view>
							</view> -->
							<view class="li_btn">
								<view class="btn_l">时间</view>
								<view class="btn_r">{{ item.create_time }}</view>
							</view>
							<!-- <view style="width: 636upx;height: 2upx;background: #EDEEF7;"></view> -->
							<view class="li_edit">
								<!-- <view class="edit_l" @click.stop="delPhoto(item.session_code)">删除</view> -->
								<view class="edit_l" @click.stop="play(item.speech_url)">播放</view>
								<view class="edit_l" @click.stop="saveShare(item.speech_url)">分享</view>
								<!-- <view class="edit_l" @click="checkDetail(item.session_code)">查看详情</view> -->
							</view>
						</view>
					</view>
				</view>
        ...
        <u-popup :show="voiceShow" mode="center" :round="10" @close="voiceShow = false" :closeOnClickOverlay="false">
        	<view class="pop_cont">
	...
	export default {
		data() {
			return {
				...
				delImg: global.baseImg + '/xcx/com/message_center/trash.png',
				right_img:global.baseImg +'/xcx/com/message_center/chevron-right.png',
		...
		methods: {
			...
			checkDetail(code) {
				uni.navigateTo({
					url: '/childPage/speech/speech_merge?h_code=' + code
				})
			},
