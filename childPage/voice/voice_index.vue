		...
		<view class="cont_title">
			...
			<view class="cont_history">
				<view style="display: flex;justify-content: space-between;">
					...
					<view class="history_title" style="color: #1F64FF;" @click="tipChange">文档格式规范</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			tipChange(){
				uni.showModal({
					content:'支持WAV、MP3、MP4、M4A、WMA、AAC、OGGAMR、FLAC格式录音文件识别音频文件大小不超过512MB，视频文件大小不超过2GB。',
					showCancel:false,
				})
			},
			// 前往实时录音
			...
			toVido() {
				...
				uni.chooseMedia({
				  ...
				  success(res) {
					...
					if(size > 536870912 && type == 'file') { // 音频文件最大只能上传512M
						that.$api.msg('该文件已超过512M，不能上传')
						return
					}
					
					if(size > 2147483648 && type == 'video') { // 视频文件最大只能上传2G
						that.$api.msg('该视频已超过2Gb，不能上传')
						return
					}
