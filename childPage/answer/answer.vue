<template>
	<view>
		...
		<view style="padding:30upx;height: 79vh;">
			<view class="title">
				<text class="title_l">*</text>
				<text class="title_r">请选择问题类型</text>
			</view>
			<view class="ans_list">
				<view class="ans_item" v-for="(item,index) in list" :key="index" @click="question(index)"
					:style="[{background:(active==index ? 'rgba(31,100,255,0.15)':'#fff'),color:(active == index ? '#1F64FF':'#333')}]">
					{{item}}
				</view>
			</view>
			<view class="title">
				<text class="title_l">*</text>
				<text class="title_r">问题详情描述</text>
			</view>
			<view class="qs_cont" style="height: 100rpx;">
				<u-textarea v-model="qs_cont" autoHeight height="200upx" border="none"
					placeholder="请填写问题内容,以便我们为您提供帮助" autoHeight></u-textarea>
			</view>
			<view class="title">
