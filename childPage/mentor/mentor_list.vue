			...
			<scroll-view class="list_ul" scroll-y="true" v-if="infoList.length > 0">
				<view class="cont_list" v-for="(item, index) in infoList" :key="index">
					<view class="list_li">
						...
						<view class="li_edit">
							<view class="edit_l" @click="mentorChat(item.avatar, item.greeting,item.me_id,item.tutor_id)">对话</view>
...
<script>
    import { getTutor, delTutor, creatTutor, getMe, getAllMe, editTutor } from '@/apis/user.js'
	export default {
		data() {
			return {
				list: [{
		...
		methods: {
			...
			toCreate() {
				uni.navigateTo({
					url: '/childPage/mentor/add_list?me_id='+ this.me_id+ '&sort='+ this.sort+ '&edit_id=' + this.tutor_id,
				})
			},
