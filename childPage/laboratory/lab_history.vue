<template>
	<view>
		<view class="top_cont">
			<view class="search_cont">
				...
				<view class="cont_tabs">
					<view class="tabs_l">
						<view @click="version_top(0)" :class="[active_top == 0 ? 'active_tabs_top' : 'tabs_top']">AI对话
						</view>
					</view>
					<view class="tabs_l">
						<view @click="version_top(1)" :class="[active_top == 1 ? 'active_tabs_top' : 'tabs_top']">AI绘画
						</view>
					</view>
				...
				<view class="chat_type" v-if="active_top == 1">
					<view @click="version(9)" :class="[active == 9 ? 'active_tabs' : 'tabs']" style="width: 32%;">Stable
						Diffusion</view>
		...
		<view class="chat_cont">
			<scroll-view class="list_ul" scroll-y="true" v-if="chatList.length > 0&&active != 6"
				:style="[{marginTop:(active_top == 1? '340rpx':'270rpx')}]">
				<!-- <scroll-view class="list_ul" scroll-y="true" v-if="chatList.length > 0" style="margin-top: 268rpx;"> -->
				<view class="ul_style" v-if="active != 6" v-for="(item, index) in chatList" :key="index"
			...
			<scroll-view class="list_ul" scroll-y="true" v-if="mjChatList.length > 0&&active == 6"
				:style="[{marginTop:(active_top == 1? '340rpx':'270rpx')}]">
				<!-- <scroll-view class="list_ul" scroll-y="true" v-if="chatList.length > 0" style="margin-top: 268rpx;"> -->
				<view class="ul_style" v-if="active == 6" v-for="(item, index) in mjChatList" :key="index"
					@click="checkDetail(item.image_code)">
					<view class="li_style">
						<view class="li_left" v-if="active_top == 1">
							<image class="li_img" :src="getImgUrl(item.result_image)"></image>
						</view>
						<view class="li_right">
							<view class="li_top">
								<text class="top_l">{{ item.title }}</text>
		...
		methods: {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							if (that.active == 0 || that.active == 1 || that.active == 4 || that.active == 5 ||
								that.active == 7 || that.active == 8 || that.active == 10 || that.active == 12
							) {
								uni.navigateTo({
			...
			getMjChatList() {
				...
				sdDrawList(val).then(res => {
					if (res.code == 20000) {
						this.mjChatList = res.data.data;
						console.log(this.mjChatList);
						let total = res.data.total;
						if (total % this.page_count == 0) {
							this.pageTotal = total / this.page_count
						} else {
