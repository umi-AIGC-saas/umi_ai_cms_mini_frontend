<template>
	<view>
		<view class="head_index">
			<view class="head_cont">
				...
				<view class="cont_tabs"
					...
					<view class="right_text" @click="toHistory">历史记录</view>
				</view>
				<!-- <view class="cont_tabs"
					style="width: 100%; background-color: #fff; display: flex; align-items: center; justify-content: space-between;">
		...
		<view :style="[{marginTop: topHeight +'px'},{marginBottom:'100rpx'}]">
			<view class="cont_tabs_two" v-if="tabIndex == 1">
				<view class="tabs_l_two">
					<view @click="version(1)" :class="[active == 0 ? 'active_tabs_two' : 'tabs_two']">DALL·E3</view>
				</view>
				<view class="tabs_l_two">
					<view @click="version(2)" :class="[active == 1 ? 'active_tabs_two' : 'tabs_two']">Stable Diffusion
					</view>
				</view>
				<view class="tabs_l_two">
					<view @click="version(3)" :class="[active == 2 ? 'active_tabs_two' : 'tabs_two']">Midjourney</view>
				</view>
			</view>
			...
			<view class="ex_ul" v-if="sd_msgLoad && chat_type == 9 && !askShow && isuser && sd_photo_type == 2">
				<view class="ex_time">
					...
					<view class="wait_text">生成中,耗时{{ sd_timer+three_num }}秒</view>
				...
				<view class="query_li" v-if="query_type == 3">
					...
					<view class="li_select_imp">
						<view class="select_cont">
							...
							<rich-text class="select_bot" v-if="select1">{{content1}}</rich-text>
						</view>
					</view>
					<view class="li_select_imp">
						<view class="select_cont">
							...
							<view class="select_right">
								<u-number-box v-model="setObj.steps" button-size="36" color="#ffffff" bgColor="#2979ff"
									:min="10" :max="50" iconStyle="color: #fff">

									<input v-model="setObj.steps" slot="input" style="width: 500rpx;text-align: center;"
										class="select_int" placeholder="选填数字，范围：10~50"
							...
							<rich-text class="select_bot" v-if="select2">{{content2}}</rich-text>
						</view>
					</view>
					...
					<view class="li_select_imp">
						<view class="select_cont">
							...
							<rich-text class="select_bot" v-if="select3">{{content3}}</rich-text>
						</view>
					</view>
					<view class="li_select_imp">
						<view class="select_cont">
							...
							<view class="select_right" style="position: relative;">
								<u-number-box v-model="setObj.seed" button-size="36" color="#ffffff" bgColor="#2979ff"
									:min="0" :max="4294967295" iconStyle="color: #fff">
									<view slot="minus" class="minus"><u-icon name="minus" size="20"></u-icon></view>
									<input v-model="setObj.seed" slot="input" style="width: 500rpx;text-align: center;"
										class="select_int" placeholder="选填数字，范围0~4294967295"
							...
							<rich-text class="select_bot" v-if="select4">{{content4}}</rich-text>
						</view>
					</view>
	...
	export default {
		...
		onShow() {
			...
			if (uni.getStorageSync('cameraInfo') != '') {
				...
				this.origin_image = this.imgList[0]
		...
		methods: {
			...
			getDrawRecommend() {
				getDrawRecommend().then(res => {
					if (res.code == 20000) {
						this.billList = res.data[Math.floor(Math.random() * 22)];
			...
			async mjDraw() {
				...
				if (this.queryIndex == 0) {
					url = this.modelList[0].pic_url
				} else {
					url = this.modelList[1].pic_url
				}

				let data = {
					"prompt": this.msg,
					"prompt_en": this.enText,
					"chat_type": '6',
					"action_type": 3,
					"size": this.size,
					// "file_list":[url]//不带域名
				...
				try {
					...
					if (id.code == 20000) {
						let request_count = 0
						let mj_draw_time = setInterval(async () => {
							if (request_count < 101) {
								...
								this.$api.msg('等待时间过长，请稍后前往历史记录查看')
								clearInterval(mj_draw_time)
								clearInterval(that.mj_t_time)
							}
						}, 2000)
											...
											if (val.data[0].status == 0) {
												...
												if (num == 6) {
													uni.setStorageSync('session_code6', val.data[0]
														.session_code);
													uni.setStorageSync('chat_group_code6', val
														.data[0]
														.chat_group_code);
												}
												if (num == 9) {
													...
													uni.setStorageSync('session_code9', val.data[0]
														.session_code);
													uni.setStorageSync('chat_group_code9', val
														.data[0]
														.chat_group_code);
												}
			...
			stopStart() {
				...
				clearInterval(this.mj_t_time)
				clearInterval(this.mj_draw_time)
				this.mj_msgLoad = false
				this.isMj = true
				
				clearInterval(this.e_t_time)
				this.e_msgLoad = false
				this.isE = true
			},
			...
			creatChatFun(data, state) {
				...
				this.requestTask_three = uni.request({
					// url: global.baseUrl + '/api/chat/new_chat_image',
				...
				// console.log(1,this.requestTask_three);
				let strArr = []
				let info = ''
				let integral = 0
