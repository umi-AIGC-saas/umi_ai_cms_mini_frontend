	...
	<view style="background-color: #fff;">
		...
		<view>
			<view class="cont_title">
				...
				<view class="title_t">
					<text class="t_left">我能给你预测宝宝未来模样</text>
				</view>
			</view>
				<view style="text-align: center;">
					<image :src="ccbb" class="ccbb"></image>
				</view>
			<view class="query_type" style="margin-left: 14%;margin-top: -20rpx;font-size: 24rpx;">
				<view class="tabs">提醒：仅供娱乐，不做最终参考</view>
			</view>
			<!-- 图片示例  参考图-->
			<view class="ex_ul" v-if="!sd_msgLoad && StableDiffusionList.length == 0 && isuser">
				<view class="ex_time" v-if="imgList.length == 0">
					...
					<view class="upload_text">点击上传小宝宝的四维彩超图哦</view>
					<view class="upload_text" style="color: #1F64FF;">(每次生成消耗5w算力)</view>
				</view>
			...
			<view class="ex_ul" v-for="(item,index) in StableDiffusionList" :key="index"
				v-if="!sd_msgLoad && StableDiffusionList.length > 0 && isuser">
				<view class="ex_img" v-for="(val, tip) in item.image_urls" :key="tip">
					...
					<view class="ex_like ex_upload" @click="savePhoto(item.image_urls[0].url)" v-if="item.is_mod != 1">
	...
	export default {
		...
		data() {
			return {
				...
				ccbb:global.baseImg+'/xcx/com/message_center/彩超baby.png',
				draw_tip: false,
...
<style scoped lang="less">
	...
	.query {
		padding: 30upx 20upx 20upx 60upx;
		background: #fff;
		margin-top: 20upx;
